<x-app-layout>
        ...
        <div class="font-semibold text-xl text-gray-800 leading-tight">
            <div class="sm:flex justify-between">
                ...
                <div class="py-1">
                    ...
                    <button data-modal-toggle="supprimerfacture"
                        class="inline-flex items-center btn btn-red transition">
                        <span class="mr-3">
                            Annuler
                        </span>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
    ...
    @section('content')
        ...
        <div id="supprimerfacture" tabindex="-1"
                ...
                <div class="relative sm-modal-content bg-white rounded-lg shadow dark:bg-gray-700 md:w-1/2">
                    ...
                    <div class="flex justify-end p-2">
                        <button type="button"
                            ...
                            data-modal-toggle="supprimerfacture">
                    ...
                    <div class="p-6 pt-0 text-center">
                        ...
                        <form action="{{ route('facture.destroy', $facture->id) }}" method="post">
                            ...
                            <button data-modal-toggle="supprimerfacture" type="button"
                                ...
                                <table>
                                    <tr>
                                        <th
                                            ...
                                            N&#176; facture</th>
                                        <td class="px-6 py-4 text-right w-full"><a
                                                href="mailto:{{ $facture->num_facture }}">{{ $facture->num_facture }}</a>
                                        </td>
                                    </tr>
                                    @if ($facture->date_echeance)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 text-right w-full">
                                                {{ strftime('%d %B %Y', strtotime($facture->date_echeance)) }}</td>
                                        </tr>
                                    @endif
                                    @if ($facture->tva)
                                        <tr>
                                        ...
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 flex justify-end w-full">
                                                @switch($facture->statut_paiment)
                                                    @case('non-payee')
                                                        <div class="bg-orange-500 font-bold py-1 px-3 rounded text-white text-center"
                                                    ...
                                                    @case('payee')
                                                        <div class="bg-green-500 font-bold py-1 px-3 rounded text-white text-center"
                                                            style="width: fit-content">
                                                            Payée
                                                        </div>
                                                    @break
                        ...
                        <div class="bg-white shadow-lg rounded-lg p-6" style="height: fit-content">
                            ...
                            <div class="welcome-img">
                                @if (pathinfo($facture->fichier_attacher)['extension'] == 'pdf')
                                    <iframe width="100%" style="height: 100mm" csp="true" allowfullscreen="true"
                                        src="/uploads/{{ $facture->fichier_attacher }}" frameborder="0"></iframe>
                                @else
                                    <img width="100%" src="/uploads/{{ $facture->fichier_attacher }}"
                                        class="rounded-md">
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="relative overflow-x-auto">
                            <div class="py-3 px-3">
                                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                    <thead
                                        class="text-xs text-white uppercase bg-indigo-500 dark:bg-gray-700 dark:text-gray-400">
                                        <tr>
                                        ...
                                        @foreach ($facture->ventes as $vente)
                                            <tr
                                                ...
                                                <td class="w-4 px-4">
                                                    <div class="flex items-center">
                                                        @if ($vente->statut == 'Éditer' &&
                                                            auth()->user()->roles()->where('slug', 'expediteur')->exists())
                                                            <input form="validerVentes" id="{{ $vente->id }}"
                                                                value="{{ $vente->id }}"
                                                                name="ventes[{{ $vente->id }}][checked]"
                                                                type="checkbox"
                                                                class="checkboxs w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                                                        @endif
                                                    </div>
                                                </td>
                                                <td scope="row"
                                                ...
                                                <td class="px-6 py-4">
                                                    @if ($vente->statut == 'Éditer' &&
                                                        auth()->user()->roles()->where('slug', 'expediteur')->exists())
                                                        <form action="{{ route('vente.validerVente', $vente->id) }}"
                                                            method="post" id="validerVente">
                                                            @csrf
                                                            <input form="validerVentes" type="date"
                                                                value="{{ old('date_livraison') ? old('date_livraison') : (new DateTime())->format('Y-m-d') }}"
                                                                name="ventes[{{ $vente->id }}][date_livraison]"
                                                                id="date_livraison_{{ $vente->id }}"
                                                                class="focus:ring-indigo-500 w-full text-gray-500 focus:border-indigo-500 rounded-md sm:text-sm border-gray-300">
                                                        </form>
                                                    @else
                                                ...
                                                <td class="px-6 flex justfy-between" style="padding: 1.5rem 0">
                                                    <a href="{{ route('vente.show', $vente->id) }}"
                                                        class="px-3 text-indigo-500 transition">
                    ...
                    <div class="mx-auto px-6 py-12 bg-white border-0 shadow-lg rounded-lg">
                        <div class="font-bold px-2 py-2 text-lg">
                            <div class="flex justify-between items-center">
                                ...
                                <button class="inline-flex items-center btn btn-orange transition"
                                        ...
                                        <tr class="top">
                                            <td colspan="6">
                                                <table>
                                                    <tr>
                                                        <td class="title">
                                                            <img class="w-16 h-16" src="/images/stock_logo.png"
                                                                alt="">
                                                        </td>
                                                        <td>
                                                            Facture N&#176; {{ $facture->num_facture }}<br />
                                                            Créé le :
                                                            {{ strftime('%d %B %Y', strtotime($facture->created_at)) }}<br />
                                                            Échéance :
                                                            {{ strftime('%d %B %Y', strtotime($facture->date_echeance)) }}
                                                        </td>
                                                    </tr>
                                                </table>
                                            </td>
        ...
        @section('script')
            <script>
                function imprimerFacture() {
                    ...
                    printwin.document.write('<!DOCTYPE html><html>' + document.getElementsByTagName("head")[0].innerHTML + document
                        .getElementById("invoice-box").innerHTML + '</html>');
                    printwin.print();
                }
            </script>
        @endsection
